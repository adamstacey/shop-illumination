...
use ShopIllumination\AdminBundle\Manager\ImageManager;
...
class ImageApiController extends Controller
{
    ...
    public function getMultipleAction(Request $request, $ids, $format)
    {
        $filesArray = array();
        $em = $this->getDoctrine()->getManager();
        $images = $em->createQuery("SELECT i FROM ShopIllumination\AdminBundle\Entity\Image i WHERE i.id IN (?1)")
    ...
    public function newAction(Request $request, $format)
    {
        ...
        $manager = $this->get('shop_illumination_admin.manager.image');
        $filesArray = array();
