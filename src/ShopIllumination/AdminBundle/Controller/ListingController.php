...
class ListingController extends Controller
{
	...
	public function indexAction(Request $request, $departmentId = null, $brandId = null, $admin = false, $all = false)
    {
         ...
         * @var $department \ShopIllumination\AdminBundle\Entity\Department
         * @var $brand \ShopIllumination\AdminBundle\Entity\Brand
         * @var $departmentToFeature \ShopIllumination\AdminBundle\Entity\DepartmentToFeature
         */
        ...
        if ($departmentId)
        {
            $department = $em->getRepository("ShopIlluminationAdminBundle:Department")->find($departmentId);
        }
        ...
        try {
            ...
            $pagination->setTemplate('ShopIlluminationAdminBundle:Includes:pagination.html.twig');
            $pagination->setSortableTemplate('ShopIlluminationAdminBundle:Includes:sortable.html.twig');
        ...
        return $this->render('ShopIlluminationAdminBundle:Listing:Templates/'.$template.'.html.twig', array(
            'brand' => $brand,
	...
	public function searchAction(Request $request, $departmentId = null, $brandId = null, $all = false)
    {
        ...
        {
            $em = $this->getDoctrine()->getManager();
            $product = $em->getRepository('ShopIllumination\AdminBundle\Entity\Product\Variant')->findOneBy(array(
        ...
        if ($request->isXmlHttpRequest()) {
            ...
            return $this->render('ShopIlluminationAdminBundle:Listing:search.html.twig', array(
                'query' => $query,
                'facets' => $facets,
                'featureGroups' => $featureGroups,
                'pagination' => $pagination,
    ...
    public function departmentFeedAction()
    {
        ...
        $routing = $em->getRepository("ShopIllumination\AdminBundle\Entity\Routing")->findAll();
    ...
    public function departmentTreeAction(Request $request, $departmentId = null, $brandId = null)
    {
        ...
        if ($departmentId)
        {
            if($brandId)
            {
                $qb = $em->getRepository("ShopIlluminationAdminBundle:Department")->createQueryBuilder('d');
                $departments = $qb->join('ShopIllumination\AdminBundle\Entity\BrandToDepartment', 'btd', Expr\Join::WITH, $qb->expr()->eq('btd.department', 'd.id'))
                ...
                $departments = $em->getRepository("ShopIlluminationAdminBundle:Department")->findBy(array('id' => $departmentId, 'status' => 'a'), array('displayOrder' => 'ASC'));
            }
        } else {
            if($brandId)
            {
        ...
        $response = $this->render('ShopIlluminationAdminBundle:Listing:departmentTree.html.twig', array(
    ...
    public function popularBrandsAction(Request $request, $departmentId = null)
    {
        ...
        $response = $this->render('ShopIlluminationAdminBundle:Listing:popularBrands.html.twig', array(
            'brands' => $brands,
    ...
    public function cheaperAlternativeAction(Request $request, $productId = null)
    {
        ...
        $product = $em->getRepository("ShopIlluminationAdminBundle:Product")->find($productId);

        $response = $this->render('ShopIlluminationAdminBundle:Listing:cheaperAlternative.html.twig', array(
            'product' => $product,
        ));
    ...
    public function popularProductsAction(Request $request, $departmentIds = null, $brandIds = null)
    {
        ...
        {
            $department = $em->getRepository("ShopIlluminationAdminBundle:Department")->find($departmentIds);
        }
        if ($brandIds !== null && !is_array($brandIds))
        {
            $brand = $em->getRepository('ShopIllumination\AdminBundle\Entity\Brand')->find($brandIds);
        }

        $products = $this->getPopularProducts($departmentIds, $brandIds, 5);

        $response = $this->render('ShopIlluminationAdminBundle:Listing:popularProducts.html.twig', array(
            'products' => $products,
            'department' => $department,
            'brand' => $brand,
        ));
    ...
    private function getPopularBrands($departmentIds = null, $num = null)
    {
        ...
        if (!$departmentIds)
        {
            ...
            $qb->select('b, count(op.id) AS total')
                ->from('ShopIllumination\AdminBundle\Entity\Brand', 'b')
                ->join('ShopIllumination\AdminBundle\Entity\Product', 'p', Expr\Join::WITH, $qb->expr()->eq('p.brand', 'b.id'))
                ->join('ShopIllumination\AdminBundle\Entity\Order\Product', 'op', Expr\Join::WITH, $qb->expr()->eq('op.product', 'p.id'))
                ->groupBy('op.product')
    ...
    private function getPopularProducts($departmentIds = null, $brandIds = null, $num = null)
    {
        ...
        if (!$departmentIds)
        {
            ...
            $qb->select('p, count(op.id) AS total')
                ->from('ShopIllumination\AdminBundle\Entity\Product', 'p')
                ->join('ShopIllumination\AdminBundle\Entity\Order\Product', 'op', Expr\Join::WITH, $qb->expr()->eq('op.product', 'p.id'))
                ->groupBy('op.product')
                ->orderBy('p.accessory', 'ASC')
                ->addOrderBy('total', 'DESC');
                ...
                foreach ($departmentParts as $departmentPart)
                {
                    ...
                    if (count($ids) > 0)
                    {
                        ...
                        $departmentProducts = $qb->select('p, count(op.id) AS total')
                            ->from('ShopIllumination\AdminBundle\Entity\Product', 'p')
                            ->leftJoin('ShopIllumination\AdminBundle\Entity\Order\Product', 'op', Expr\Join::WITH, $qb->expr()->eq('op.product', 'p.id'))
