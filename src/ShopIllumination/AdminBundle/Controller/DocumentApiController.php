...
class DocumentApiController extends Controller
{
    ...
    public function getMultipleAction(Request $request, $ids, $format)
    {
        $filesArray = array();
        $em = $this->getDoctrine()->getManager();
        $documents = $em->createQuery("SELECT d FROM ShopIllumination\AdminBundle\Entity\Document d WHERE d.id IN (?1)")
    ...
    public function newAction(Request $request, $format)
    {
        ...
        $manager = $this->get('shop_illumination_admin.manager.document');
        $filesArray = array();
    ...
    {
        $em = $this->getDoctrine()->getManager();
        $document = $em->getRepository("ShopIllumination\AdminBundle\Entity\Document")->find($id);
        if (!$document)
        {
