...
class ProductApiController extends Controller
{
    ...
    public function getAction(Request $request, $format)
    {
        ...
        if($request->query->get('q', null)) {
            $query = $em->createQuery("SELECT p, pd FROM ShopIllumination\AdminBundle\Entity\Product p JOIN p.descriptions pd WHERE pd.pageTitle LIKE ?1 ");
            $query->setParameter(1, "%" . $request->query->get('q', null) . "%");
        } else {
            $query = $em->createQuery("SELECT p, pd FROM ShopIllumination\AdminBundle\Entity\Product p JOIN p.descriptions pd");
        }
    ...
    public function updatePriceAction(Request $request, $id, $format)
    {
        ...
        $this->get('shop_illumination_admin.manager.product')->updateProduct($product);
