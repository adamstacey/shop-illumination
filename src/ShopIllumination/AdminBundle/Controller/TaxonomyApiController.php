...
use ShopIllumination\AdminBundle\Entity\Taxonomy;
...
class TaxonomyApiController extends Controller
{
    ...
    public function getAction(Request $request, $format)
    {
        ...
        if ($request->query->get('q', null)) {
            $query = $em->createQuery("SELECT t FROM ShopIllumination\AdminBundle\Entity\Taxonomy t WHERE t.name LIKE ?1");
            $query->setParameter(1, "%" . $request->query->get('q', null) . "%");
        } else {
            $query = $em->createQuery("SELECT t FROM ShopIllumination\AdminBundle\Entity\Taxonomy t");
        }
    ...
    public function getTaxonomyAction(Request $request, $id, $format)
    {
        ...
        $taxonomy = $em->getRepository("ShopIllumination\AdminBundle\Entity\Taxonomy")->find($id);
