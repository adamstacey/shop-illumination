...
use ShopIllumination\AdminBundle\Form\Product\EditProductDepartmentsType;
use ShopIllumination\AdminBundle\Form\Product\EditProductDocumentsType;
use ShopIllumination\AdminBundle\Form\Product\EditProductImagesType;
use ShopIllumination\AdminBundle\Form\Product\EditProductDescriptionType;
use ShopIllumination\AdminBundle\Form\Product\EditProductSeoType;
use ShopIllumination\AdminBundle\Model\ImportPriceData;
...
use ShopIllumination\AdminBundle\Form\Product\EditProductLinksType;
use ShopIllumination\AdminBundle\Form\Product\EditProductOverviewType;
use ShopIllumination\AdminBundle\Form\Product\NewProductFeatureGroupsType;
use ShopIllumination\AdminBundle\Form\Product\NewProductFeaturesType;
use ShopIllumination\AdminBundle\Entity\Department;
use ShopIllumination\AdminBundle\Entity\Product;
use ShopIllumination\AdminBundle\Entity\Product\Description;
use ShopIllumination\AdminBundle\Entity\Product\FeatureGroup;
...
class ProductController extends Controller {
    public function viewAction(Request $request, $id, $variant=null)
    {
        ...
        /**
         * @var \ShopIllumination\AdminBundle\Entity\Product $product
         */
            ...
            foreach ($entity->getFeatures() as $feature)
            {
                if ($feature && $feature->getFeatureGroup() && $feature->getFeature())
                {
                    ...
                    $departmentToFeature = $em->createQuery("
                                SELECT dtf
                                FROM ShopIllumination\AdminBundle\Entity\DepartmentToFeature dtf
        ...
        $guarantees = $em->getRepository("ShopIllumination\AdminBundle\Entity\Guarantee")->findBy(array(
        ...
        $images = $em->getRepository("ShopIllumination\AdminBundle\Entity\Product\Image")->findBy(array(
            'objectId' => $id
        ), array(
            'displayOrder' => 'ASC'
        ));
        ...
        if ($variant)
        {
            $images = array_merge($em->getRepository("ShopIllumination\AdminBundle\Entity\Product\Variant\Image")->findBy(array(
        ...
        {
            $brandDepartmentProductsQuery = $em->createQuery("SELECT p, pd FROM ShopIllumination\AdminBundle\Entity\Product p JOIN p.departments pd WHERE p.brand = :brand AND pd.department = :department AND p.id != :id")
            ...
            if ($brandDepartmentProducts)
            {
                ...
                if ($relatedProductTotal > 0)
                {
                    $departmentProductsQuery = $em->createQuery("SELECT p, pd FROM ShopIllumination\AdminBundle\Entity\Product p JOIN p.departments pd WHERE pd.department = :department AND p.id != :id")
                        ->setParameter('department', $product->getDepartment()->getDepartment())
                        ->setParameter('id', $product->getId())
                        ->setMaxResults($relatedProductTotal);
        ...
        {
            $panOffers = array();
            $panOffers[] = $em->getRepository("ShopIllumination\AdminBundle\Entity\Product")->find(5068);
            $panOffers[] = $em->getRepository("ShopIllumination\AdminBundle\Entity\Product")->find(5069);
        }

        return $this->render('ShopIlluminationAdminBundle:Product:Templates/'.$template.'.html.twig', array(
            'product' => $product,
    ...
    public function viewWithVariantAction(Request $request, $id)
    {
        ...
        $variant = $em->getRepository("ShopIllumination\AdminBundle\Entity\Product\Variant")->find($id);
        if (!$variant)
        {
    ...
    public function newAction(Request $request, $departmentId = null, $brandId = null, $admin = false)
    {
        ...
        $flow = $this->get('shop_illumination_admin.form.flow.new_product');
        $flow->bind($product);
        ...
        return $this->render('ShopIlluminationAdminBundle:Product:new.html.twig', array(
    ...
    public function cloneAction(Request $request, $productId)
    {
        ...
        $originalProduct = $em->getRepository("ShopIllumination\AdminBundle\Entity\Product")->find($productId);
        if(!$originalProduct)
        {
            throw new NotFoundHttpException("Product not found");
        }
        ...
        $flow = $this->get('shop_illumination_admin.form.flow.clone_product');
        ...
        return $this->render('ShopIlluminationAdminBundle:Product:clone.html.twig', array(
            'form' => $form->createView(),
            'flow' => $flow,
    ...
    public function newFeatureGroupsAction(Request $request, $admin=false)
    {
        ...
        {
            $department = $em->getRepository("ShopIllumination\AdminBundle\Entity\Department")->find($request->query->get('departmentId'));
        }

        // Check for a product
        if ($request->query->has('productId'))
        {
            $product = $em->getRepository("ShopIllumination\AdminBundle\Entity\Product")->find($request->query->get('productId'));
        }
            ...
            if ($form->isValid()) {
                ...
                foreach ($featureGroups as $featureGroup)
                {
                    ...
                    if ($department)
                    {
                        ...
                        $departmentToFeatures = $em->getRepository("ShopIllumination\AdminBundle\Entity\DepartmentToFeature")->findBy(array('department' => $department));
                    ...
                    if ($product)
                    {
                        ...
                        $variants = $em->getRepository("ShopIllumination\AdminBundle\Entity\Product\Variant")->findBy(array('product' => $product));
                        foreach ($variants as $entity)
                        {
                            // Get the other feature groups
                            $variantToFeatures = $em->getRepository("ShopIllumination\AdminBundle\Entity\Product\VariantToFeature")->findBy(array('variant' => $entity));
        ...
        return $this->render('ShopIlluminationAdminBundle:Product:newFeatureGroups.html.twig', array(
            'form' => $form->createView(),
        ));
    }
    ...
    public function newFeaturesAction(Request $request, $admin=false)
    {
        ...
        return $this->render('ShopIlluminationAdminBundle:Product:newFeatures.html.twig', array(
            'form' => $form->createView(),
        ));
    }
    ...
    public function importPricesAction(Request $request)
    {
                ...
                foreach ($file as $line) {
                    if (sizeof($line) >= 2)
                    {
                        ...
                        if ($importContent['productCode'] && ($importContent['newPrice'] > 0))
                        {
                             ...
                             * @var $variant Variant
                             */
                            $variant = $em->getRepository('ShopIllumination\AdminBundle\Entity\Product\Variant')->findOneBy(array('productCode' => $importContent['productCode']));
                            if ($variant)
                            {
        ...
        return $this->render('ShopIlluminationAdminBundle:Product:importPrices.html.twig', array(
            'form' => $form->createView(),
            'processed' => $processed,
    ...
    {
        $em = $this->getDoctrine()->getManager();
        $brands = $em->getRepository("ShopIllumination\AdminBundle\Entity\Brand\Description")->findBy(array(), array('name' => 'ASC'));

        $brandId = $request->query->get('brandId');
        if ($brandId)
        {
            $brand = $em->getRepository("ShopIllumination\AdminBundle\Entity\Brand")->find($brandId);
        }
        if ($brand)
        {
            $productsQuery = $em->createQuery("SELECT p FROM ShopIllumination\AdminBundle\Entity\Product p WHERE p.brand = :brand")
        ...
        return $this->render('ShopIlluminationAdminBundle:Product:exportVisualSoftProducts.html.twig', array('brands' => $brands));
    }

    public function baseEditAction(Request $request, $productId, $template, $formClass)
    {
        $em = $this->getDoctrine()->getManager();
        $manager = $this->get('shop_illumination_admin.manager.product');

        $product = $em->getRepository("ShopIllumination\AdminBundle\Entity\Product")->find($productId);
        if (!$product)
        {
            throw new NotFoundHttpException("Product not found");
        }
    ...
    public function editOverviewAction(Request $request, $productId)
    {
        ...
        return $this->render('ShopIlluminationAdminBundle:Product:edit_overview.html.twig', array(
            'product' => $product,
            'form' => $form->createView(),
        ));
    }
    ...
    {
        return $this->baseEditAction($request, $productId, 'ShopIlluminationAdminBundle:Product:edit_description.html.twig', new EditProductDescriptionType());
    }
    ...
    public function editDepartmentsAction(Request $request, $productId)
    {
        ...
        return $this->render('ShopIlluminationAdminBundle:Product:edit_departments.html.twig', array(
            'product' => $product,
            'form' => $form->createView(),
        ));
    }
    ...
    {
        return $this->baseEditAction($request, $productId, 'ShopIlluminationAdminBundle:Product:edit_seo.html.twig', new EditProductSeoType());
    }
    ...
    public function editImagesAction(Request $request, $productId)
    {
        ...
        return $this->render('ShopIlluminationAdminBundle:Product:edit_images.html.twig', array(
            'product' => $product,
            'form' => $form->createView(),
        ));
    }
    ...
    public function editDocumentsAction(Request $request, $productId)
    {
        ...
        return $this->render('ShopIlluminationAdminBundle:Product:edit_documents.html.twig', array(
            'product' => $product,
            'form' => $form->createView(),
        ));
    }
    ...
    public function editLinksAction(Request $request, $productId)
    {
        ...
        return $this->render('ShopIlluminationAdminBundle:Product:edit_links.html.twig', array(
            'product' => $product,
            'form' => $form->createView(),
        ));
    }
    ...
    public function editVariantsAction(Request $request, $productId)
    {
        ...
        return $this->render('ShopIlluminationAdminBundle:Product:edit_variants.html.twig', array(
            'product' => $product,
        ));
    }
    ...
    public function deleteAction($productId)
    {
        ...
        $em->remove($product);
        $em->flush();
        $this->get('shop_illumination_admin.indexer.product')->deleteById($productId);
     ...
     *
     * @return \ShopIllumination\AdminBundle\Manager\SeoManager
     */
    private function getSeoManager()
    {
        return $this->get('shop_illumination_admin.manager.seo');
    }
}