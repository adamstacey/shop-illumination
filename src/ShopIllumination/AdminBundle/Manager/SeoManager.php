...
class SeoManager extends Manager {
    ...
    public function updateRedirects($objectId, $objectType, $redirectFrom, $redirectTo, $secondaryId = 0)
    {
        ...
        $existingRedirects = $this->doctrine->getRepository('ShopIllumination\AdminBundle\Entity\Redirect')->findByRedirectFrom($redirectTo);
        ...
        $existingRedirects = $this->doctrine->getRepository('ShopIllumination\AdminBundle\Entity\Redirect')->findByRedirectTo($redirectFrom);
        foreach ($existingRedirects as $existingRedirect)
        {
    ...
    public function doesUrlExist($url = '')
    {
        if ($url != '')
        {
            $urlCheck = $this->doctrine->getRepository('ShopIllumination\AdminBundle\Entity\Routing')->findOneByUrl($url);
            if ($urlCheck)
            {
                return true;
            }
