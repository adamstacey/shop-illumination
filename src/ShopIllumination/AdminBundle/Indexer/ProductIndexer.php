...
class ProductIndexer extends Indexer
{
    ...
    public function createDocument(\Solarium_Query_Update $query, Product $product)
    {
        ...
        $links = $em->getRepository('ShopIllumination\AdminBundle\Entity\Product\Link')->findBy(array(
            'product' => $product->getId(),
        ));
        ...
        $departmentToFeatures = $em->createQuery("SELECT dtf FROM ShopIllumination\AdminBundle\Entity\DepartmentToFeature dtf WHERE dtf.department = ?1 ORDER BY dtf.displayOrder ASC")
        ...
        foreach ($product->getVariants() as $variant) {
            ...
            $variantImages = $em->getRepository('ShopIllumination\AdminBundle\Entity\Product\Variant\Image')->findBy(array(
