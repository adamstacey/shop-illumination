...
class BuildSitemapCommand extends ContainerAwareCommand
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $routeManager = $this->getContainer()->get('shop_illumination_admin.manager.routing');
        $result = $em->createQuery("SELECT r FROM ShopIllumination\\AdminBundle\\Entity\Routing r")->iterate();
        while (($row = $result->next()) !== false)
        {
            ...
            try {
                ...
                // Calculate priority
                if (get_class($route) === 'ShopIllumination\\AdminBundle\\Entity\\Brand\\Routing' ||
                    get_class($route) === 'ShopIllumination\\AdminBundle\\Entity\\Product\\Routing' ||
                    get_class($route) === 'ShopIllumination\\AdminBundle\\Entity\\Product\\Variant\\Routing')
                {
                    $priority = '1.0';
                } else {
                    $priority = '0.8';
