...
class FixMaiaProductsCommand extends ContainerAwareCommand
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        // Fetch Department
        $brand = $em->getRepository('ShopIlluminationAdminBundle:Brand')->find(15);
        $department = $em->getRepository('ShopIlluminationAdminBundle:Department')->find(41);
        $oldProducts = $em->getRepository('ShopIllumination\AdminBundle\Entity\Product')->findBy(array('brand' => 15));
        $colours = array();
        ...
        foreach($colours as $colour)
        {
            ...
            // Add route
            $url = $this->getContainer()->get('shop_illumination_admin.manager.seo')->createUrl('maia-worktops/' . $colour);
